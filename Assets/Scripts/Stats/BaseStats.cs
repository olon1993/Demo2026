...
namespace RPG.Stats
{
    ...
    public class BaseStats : MonoBehaviour
    {
        ...
        private int GetStatModifier(Stat stat)
        {
            ...
            int statModifierSum = 0;
            foreach(IModifierProvider provider in GetComponents<IModifierProvider>())
            {
                foreach(int modifier in provider.GetStatModifiers(stat))
                {
        ...
        private int GetStatMultiplier(Stat stat)
        {
            if(_shouldUserModifiers == false)
            {
                return 1;
            ...
            int statMultiplierSum = 100;
            foreach (IModifierProvider provider in GetComponents<IModifierProvider>())
            {
                foreach (int multiplier in provider.GetStatMultipliers(stat))
                {
